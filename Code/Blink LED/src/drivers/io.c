 ...
 * (GCC) or "--enum_type=packed" (TI), the enums are represented as a single
 * byte (8-bit), so given that the pins are ordered in increasing order
 * (see io_generic_e), and that there are 3 ports and 8 pins, the enum value
 * can be viewed as:
 * [ Zeros (3 bits) | Port (2 bits) | pin (3 bits) ] */
#ifndef static_assert
#define static_assert _Static_assert
#endif
static_assert(sizeof(io_generic_e) == 1, "Unexpected size, -fshort-enums/--enum_type=packed missing?");
